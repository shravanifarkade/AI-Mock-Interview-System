# ===========================
# AI Mock Interview: SBERT + LightGBM (Optimized & Fixed)
# ===========================

import os, re, pickle, random, requests
import pandas as pd
import numpy as np
...
BATCH_SIZE = 64  # SBERT batch size for embeddings
...
def extract_features(df, sbert_model, tfidf_vectorizer):
    ...
    # TF-IDF similarity using cosine_similarity
    from sklearn.metrics.pairwise import cosine_similarity as sk_cos
    q_tfidf = tfidf_vectorizer.transform(df['Question'])
    a_tfidf = tfidf_vectorizer.transform(df['Answer'])
    tfidf_sim = np.array([sk_cos(q, a)[0][0] for q,a in zip(q_tfidf, a_tfidf)]).reshape(-1,1)
...
# ---------------- Generate Labels ----------------
def generate_labels(df, sbert_model, tfidf_vectorizer):
    q_emb = embed_texts(sbert_model, df['Question'].tolist())
    a_emb = embed_texts(sbert_model, df['Answer'].tolist())

    cosine_sim = np.array([util.cos_sim(q.reshape(1,-1), a.reshape(1,-1)).item() for q,a in zip(q_emb, a_emb)])
    tfidf_sim = np.array([sk_cos(tfidf_vectorizer.transform([q]), tfidf_vectorizer.transform([a]))[0][0]
                          for q,a in zip(df['Question'], df['Answer'])])

    labels = np.zeros(len(df))
    labels[cosine_sim > 0.7] += 1
    labels[tfidf_sim > 0.6] += 1
    labels = np.clip(labels, 0, 3).astype(int)

    high_scores = np.percentile(cosine_sim, 90)
    labels[cosine_sim >= high_scores] = 3
    return labels

# ---------------- Train Model ----------------
def train_model_lgb_optimized():
    ...
    X = extract_features(combined_df, sbert_model, tfidf_vectorizer)
    y = generate_labels(combined_df, sbert_model, tfidf_vectorizer)

    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, stratify=y, random_state=42)

    # SMOTE (set k_neighbors=1 to avoid errors if class has few samples)
    ...
    clf = lgb.LGBMClassifier(
        ...
        num_class=4,
        min_child_samples=1,  # FIX: avoid -inf gain
        random_state=42
    )
...
def evaluate_answer(user_ans, ref_ans, clf, sbert_model, tfidf_vectorizer):
    ...
    cosine_sim = sk_cos(q_emb, a_emb)[0][0]
    tfidf_sim = sk_cos(tfidf_vectorizer.transform([ref_ans]), tfidf_vectorizer.transform([user_ans]))[0][0]
...
# ---------------- Interactive Demo ----------------
def run_demo_lgb_optimized():
    clf, sbert_model, tfidf_vectorizer, combined_df = train_model_lgb_optimized()
